...
function runTheRegister() {
   ...
   // try catch statement to the runTheRegister() function
   try {
      if (changeValue < 0) { // changeValue is not greater than or equal to zero
         throw "Cash amount doesn’t cover the bill"; //error message
      }

      changeBox.value = formatCurrency(changeValue); // format  change as currency
      calcChange(changeValue); // units of currency 
   }
   //e catch statement set the innerHTML of the element with the id “warning” 
   catch (error) {
